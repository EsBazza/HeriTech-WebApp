    ...
    <footer className="bg-[#141312] text-gray-300 border-t border-gray-800 mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-4 md:col-span-1">
            ...
            <div className="flex items-center space-x-2 text-xs text-emerald-400 font-mono-data">
              ...
              <span>{translateSync("Supabase PostgreSQL + Google AI Active")}</span>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white">
              {translateSync("70/20/10 Global Standard")}
            </h4>
            <ul className="space-y-2 text-xs text-gray-400">
              <li className="flex items-center space-x-2">
                <HeartHandshake className="w-3.5 h-3.5 text-blue-400" />
                <span>
                  <strong>70%</strong> {translateSync("Direct Artisan Fair Payout")}
                </span>
              </li>
              <li className="flex items-center space-x-2">
                <Globe className="w-3.5 h-3.5 text-emerald-400" />
                <span>
                  <strong>20%</strong> {translateSync("LGU & Platform Operations")}
                </span>
              </li>
              <li className="flex items-center space-x-2">
                <Leaf className="w-3.5 h-3.5 text-amber-400" />
                <span>
                  <strong>10%</strong> {translateSync("Verified Environmental NGO Trust")}
                </span>
              </li>
            </ul>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white">
              {translateSync("Pan-Asian Pilot Proofs")}
            </h4>
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-400">
              <div className="p-2 rounded bg-white/5 border border-white/5">
                <span className="font-semibold text-white">Panagbenga 🇵🇭</span>
                <p className="text-[10px] text-gray-400">{translateSync("Baguio Bamboo & Florals")}</p>
              </div>
              <div className="p-2 rounded bg-white/5 border border-white/5">
                <span className="font-semibold text-white">Yi Peng 🇹🇭</span>
                <p className="text-[10px] text-gray-400">{translateSync("Chiang Mai Lanterns")}</p>
              </div>
              <div className="p-2 rounded bg-white/5 border border-white/5">
                <span className="font-semibold text-white">Ganesh Chaturthi 🇮🇳</span>
                <p className="text-[10px] text-gray-400">{translateSync("Thane Nirmalaya Inks")}</p>
              </div>
              <div className="p-2 rounded bg-white/5 border border-white/5">
                <span className="font-semibold text-white">Pingxi 🇹🇼</span>
                <p className="text-[10px] text-gray-400">{translateSync("Mulberry Paper Guild")}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-white">
              {translateSync("System Links")}
            </h4>
            <ul className="space-y-1.5 text-xs text-gray-400">
              <li>
                <Link href="/" className="hover:text-white transition-colors">
                  {translateSync("Marketplace")}
                </Link>
              </li>
              <li>
                <Link href="/impact" className="hover:text-white transition-colors">
                  {translateSync("Public Impact Ledger")}
                </Link>
              </li>
              <li>
                <Link href="/profile" className="hover:text-white transition-colors">
                  {translateSync("Artisan & LGU Verification")}
                </Link>
              </li>
              <li>
                <Link
        ...
        <div className="mt-10 pt-6 border-t border-gray-800 flex flex-col md:flex-row items-center justify-between text-xs text-gray-500">
          <p>© 2026 HeriTech V4 • {translateSync("EDUtech Asia Planet Protectors Challenge")}</p>
          <p className="font-mono-data text-[10px] mt-2 md:mt-0">
            {translateSync("SHA-256 Provenance Ledger & Google Wallet Pass Integration")}
          </p>
        </div>
      </div>
    </footer>
